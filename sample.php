...
foreach ($rows as $row) {
    ...
    foreach ($files as $file) {
        ...
        switch ($ext) {
            case 'jpeg':
            case 'png':
                '<img class="float" style="height:350px;width:530px" src="' . $attach_url . '">';
                break;
            case 'mp4':
                '<video class="float" style="height:350px;width:530px" src="' . $attach_url . '">';
                break;
            case 'pdf':
                '<iframe class="float" style="height:350px;width:530px" src="' . $attach_url . '"></iframe>';
                break;
            default:
                break;
        }
    ...
    <div id="input_area">
        <form name="answer_Input_form">
            ...
            <style>
                ...
                /* ファイルが3件の場合 */
                .quest_container .quest_markdown_group {
                    float: none;
                }
                ...
                /* ファイルが3件の場合 */
                .quest_container .quest_item_group {
                    float: none;
...
<?php echo '</div>'; ?>
<script>
    var count = $(".float").length; // 何個の要素があるかをlengthを使ってcount
    const divQuestMarkdown = document.querySelector('quest_markdown');
    const divQuestItem = document.querySelector('quest_item');

    if (count == 1) {
        // 指定のクラスが1つのときは、指定のタグにクラス名「alone」を追加
        divQuestMarkdown.classList.remove('quest_markdown');
        divQuestItem.classList.remove('quest_item');
    } else if (count == 2) {
        // 指定のクラスが2つのときは、指定のタグにクラス名「alone」を追加
        divQuestMarkdown.classList.remove('quest_markdown');
        divQuestItem.classList.remove('quest_item');
    } else if (count == 3) {
        // 指定のクラスが2つのときは、指定のタグにクラス名「group」を追加
        divQuestMarkdown.classList.remove('quest_markdown_group');
        divQuestItem.classList.remove('quest_item_group');
    }
